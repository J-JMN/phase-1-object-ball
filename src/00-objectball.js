...
function numPointsScored(playerName) {
    let obj = gameObject();
    for (let team in obj) {
      if (obj[team].players[playerName]) {
        return obj[team].players[playerName].points;
      }
    }
  }
...
function shoeSize(playerName) {
    ...
    for (let team in obj) {
      if (obj[team].players[playerName]) {
        return obj[team].players[playerName].shoe;
      }
    }
  }
...
function playerNumbers(teamName) {
    ...
    for (let team in obj) {
      if (obj[team].teamName === teamName) {
        return Object.values(obj[team].players).map(player => player.number);
      }
    }
  }
...
function bigShoeRebounds() {
    ...
    for (let team in obj) {
      for (let player in obj[team].players) {
        if (obj[team].players[player].shoe > biggestShoe) {
          biggestShoe = obj[team].players[player].shoe;
